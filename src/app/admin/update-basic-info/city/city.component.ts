...
export class CityComponent implements OnInit {
  ...
  AddEditUi: any[] = [
    ...
    },
    {
      ...
      sortOrder: 1,
      colwidth: 'col-12 col-xl-4 col-lg-4 col-md-4 col-sm-12',
      maxLenth: '3',
      isMandatory: true,
  ...
  ngOnInitAPIDatas() {
    ...
    observable.push(this.location.getCity())
    forkJoin(observable).subscribe((resArray: any) => {
      resArray.forEach((res: any, index: number) => {
        if (res.statuscode === 200 && index === 0) {
          ...
          this.AddEditUi[5]['options'] = this.DistrictList;
          console.log("data",this.AddEditUi);
         
        }
        else if (res.statuscode === 200 && index === 3) {
  ...
  AddState() {
    console.log("add new data",this.AddEditUi);
  ...
  editState(id: any) {
    this.AddEditUi.sort((a, b) => a.sortOrder - b.sortOrder);
    this.location.geCityById(id).subscribe((res: any) => {
      this.uiAnswer = []
      if (res.statuscode === 200) {
      ...
      this.dialog.open(EditDataComponent, {
        ...
        if (res != false) {
          this.ngOnInitAPIDatas();
          this.location.updateCity(res).subscribe((res: any) => {
            if (res.statuscode === 200) {
              this.message.show(res.message, 'SUCCESS',
                {
                  status: 'success'
  ...
  deleteState(Id: number, Name:any) {
    this.dialog.open(ConfirmationPopupComponent, {
      ...
      if (res.action === true) {
        this.location.deleteCity(res.id).subscribe((res: any) => {
          if (res.statuscode === 200) {
